...
* [:open-notificaties:`336` / :open-notificaties:`340`] Use celery beat to run a task to send notifications/cloud events
  ...
  environment variables that affect this scheduling have been added (see :ref:`installation_env_config` > Celery):

    * ``NOTIFICATION_SEC_INTERVAL``: the amount of seconds between starting the ``execute_notifications`` task that creates the actual notification request tasks (minimum 5 seconds). Defaults to: ``20``.
