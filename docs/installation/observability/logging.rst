...
``debug``, ``info``, ``warning``, ``error`` or even ``critical``. By default, Open Notificaties
...
Open Notificaties emits structured logs in JSON format (unless explicitly configured otherwise),
...
``request_id``
    ...
    by background tasks or logs emitted before/after the Open Notificaties app.
