...
* ``notification_successful``: a notification was successfully forwarded to a subscribed callback URL. Additional context:
    ...
    * ``attempt_number`` the number of tasks that have been started for this notification
    * ``autoretry_attempt_number``: the number of times a specific task has been attempted
