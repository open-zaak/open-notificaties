...
# Dump component data
# This script is not intended for a data migration to another Open Notificatie instance.
...
# pg_dump also does not add related tables automatically, so `dump_data.sh datamodel` does not add related data from accounts to the dump.


set -e

DEFAULT_APPS=(datamodel)

export PGHOST=${DB_HOST:-db}
export PGPORT=${DB_PORT:-5432}
export PGUSER=${DB_USER:-opennotificaties}
export PGDATABASE=${DB_NAME:-opennotificaties}
